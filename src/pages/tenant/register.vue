    ...
    <view class="register-container">
      ...
      <view class="form-group">
        <view class="form-item">
          <uni-icons type="person" size="20" color="#4CD964"></uni-icons>
          <input 
            type="text" 
        ...
        <view class="form-item">
          <uni-icons type="phone" size="20" color="#4CD964"></uni-icons>
          <input 
            type="number" 
        ...
        <view class="field-error" v-if="fieldErrors.phone">{{ fieldErrors.phone }}</view>
      </view>
...
const handleRegister = async () => {
  ...
  // 检查是否有错误或未填写信息
  if (!formData.username || !formData.password || !formData.confirmPassword || !formData.phone || !formData.agreeTerms) {
    uni.showToast({ 
      title: '请完善注册信息', 
      icon: 'none' 
    });
  ...
  if (fieldErrors.username || fieldErrors.password || fieldErrors.confirmPassword || fieldErrors.phone || fieldErrors.terms) {
    uni.showToast({ 
      title: '请检查填写的信息', 
      icon: 'none' 
    });
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (response.token) {
          ...
          // 延迟一下跳转
          setTimeout(() => {
            // 跳转到租户主页 - 使用switchTab因为这是tabBar页面
            uni.switchTab({
...
// 跳转到登录
const goToLogin = () => {
  uni.navigateTo({ url: '/pages/tenant/login' });
};
...
.form-item {
  ...
  &:focus-within {
    ...
    box-shadow: 0 0 0 2rpx rgba(76, 217, 100, 0.1);
  }
